...
void QExcel::deleteFile(QString FileName)
{
    QFile file(FileName);
...
QColor QExcel::getCellBorderColor(int row, int column)
{
    QAxObject* range = sheet->querySubObject("Cells(int,int)", row, column);
    return range->property("Color").Color;
}

QColor QExcel::getCellBorderColor(const QString& cell)
{
    QAxObject* range = sheet->querySubObject("Range(const QString&)", cell);
    return range->property("Color").Color;
}
